...
class Custom(Dataset):
    ...
    def browse_dataroot(self, dataroot, mode, pattern):
        ...
        if os.path.exists(f"tmp/custom_{name}_{mode}.pkl"):
            mesh_paths = jt.load(f"tmp/custom_{name}_{mode}.pkl")
            return mesh_paths
        ...
        if mode in ["train","val","test","trainval"]:
            ...
            jt.save(mesh_paths, f"tmp/custom_{name}_{mode}.pkl")
    ...
    def __getitem__(self, idx):
        ...
        mesh = trimesh.load(mesh_file, process=False, maintain_order=True)
        levels, indexes, labels, knn_dis, knn_idex = jt.load(mesh_file.replace(".obj", ".pkl"))
        if labels is None:
            labels = np.zeros((len(mesh.faces),), dtype=np.int32)

        mesh = self.transforms(mesh)
        feature, Fs, center = load_feature(mesh, self.feats)
        assert Fs == len(labels), f"{Fs} != {len(labels)}"

        return feature, Fs, labels, center
