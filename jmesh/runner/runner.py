...
from jmesh.utils.registry import (
    build_from_cfg,
    MODELS,
    SCHEDULERS,
    DATASETS,
...
from jmesh.utils.general import (
    build_file,
    current_time,
    search_ckpt,
    check_file,
...
import torch
from dgnet_pytorch import DGNet


def load_jittor_npz_to_torch(torch_model, npz_path):
    np_weights = np.load(npz_path)
    ...
    for k in torch_state:
        if k in np_weights:
            new_state[k] = torch.tensor(np_weights[k])
        else:
            print(f"Missing key in Jittor weights: {k}")
            missing_keys.append(k)

    torch_model.load_state_dict(new_state, strict=False)
    return missing_keys


class Runner:
    def __init__(self):
        self.torch_model = DGNet(
        ...
        load_jittor_npz_to_torch(self.torch_model, "converted_model_weights.npz")
        ...
        self.scheduler = build_from_cfg(
            cfg.lr_scheduler, SCHEDULERS, optimizer=self.optimizer
        )
    ...
    def train(self):
        ...
        if jt.rank == 0:
            data = metric.value()
            data.update(dict(iter=self.epoch, name=self.cfg.name, is_print=True))
            self.logger.log(data)
    ...
    def val_jittor(self):
        ...
        for batch_idx, (feats, targets) in tqdm(
            ...
            desc=f"Val {self.epoch}",
            total=len(self.val_dataset),
        ):
            outputs = self.model(feats)
            metric.add(outputs.numpy(), targets)
            files.extend(feats.get("mesh_files"))
            if is_save:
                for mesh_file, o, t in zip(
                    feats.get("mesh_files"),
                    outputs.numpy(),
                    targets,
                ):
                    results.append((mesh_file, o, t))

            preds = np.argmax(outputs.numpy(), 1)
            np.savetxt("outputs_jittor.txt", preds)
    ...
    def val_torch(self):
        ...
        for batch_idx, (feats, targets) in tqdm(
            ...
            outputs = self.torch_model(feats)
            metric.add(outputs.detach().numpy(), targets)
            files.extend(feats.get("mesh_files"))
            if is_save:
                for mesh_file, o, t in zip(
                        feats.get("mesh_files"),
            ...
            preds = np.argmax(outputs.detach().numpy(), 1)
            np.savetxt("outputs_torch.txt", preds)
            ...
            for batch_idx, (feats, targets) in tqdm(
                ...
                for mesh_file, mf, o, t in zip(
                    ...
                    if mesh_file not in all_res:
                        all_res[mesh_file] = (
                            [o[:, :mf], t[:mf]]
                            if self.cfg.processor == "segmentation"
                            else [o, t]
                        )
                    else:
                        all_res[mesh_file][0] += (
                            o[:, :mf] if self.cfg.processor == "segmentation" else o
                        )
    ...
    def save_npz_model(model, filename="converted_model_weights.npz"):
        weights = model.state_dict()
        np_weights = {k: v.numpy() for k, v in weights.items()}
        np.savez(filename, **np_weights)
